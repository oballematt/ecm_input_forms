$(document).ready(() => {
  ...
  let costImpValues = $('.costPhase:contains("Implementation")')
    .map((i, el) =>
      parseInt(
        $(el)
          .siblings(".sumCosts")
  ...
  let costAnnValues = $('.costPhase:contains("Annual")')
    .map((i, el) =>
      parseInt(
        $(el)
          .siblings(".sumCosts")
  ...
  if (totalImpSum > 1) {
    ...
    $(".display").text(
      `Warning - Total Implem Fundings is less than Total Implem Costs by: $${impCommaNum} `
    );
  }
  ...
  if (totalAnnSum > 1) {
    $(".warning").removeAttr("style");
    $(".display").text(
      `Warning - Total Annual Fundings is less than Total Annual Costs by: $${annCommaNum} `
    );
  }

  $("#formData").on("change", ["#building", "#measure_type"], () => {
    const pidYear = parseInt(
      $("#reporting")
        .val()
        .slice(6),
      10
    ...
    if ($("#measure_type").val() === null) {
      ...
      $(".pid").val(
        $("#building").val() + " " + pidYear2 + " " + $("#measure_type").val()
      );
    } else {
      $(".pid").val(
        $("#building").val() + " " + pidYear + " " + $("#measure_type").val()
      );
    }
  });
  ...
  Object.keys(filterObjs).forEach(function(key) {
    ...
    $(key).on("click", function() {
      ...
      switch (check) {
        case "All":
          $(
            ".adam, .amanda, .buddy, .cedric, .dave, .grace, .john, .matt, .meagan, .pat, .richard, .travis"
          ).hide();
          break;
        case "Adam":
          $(
            ".all, .amanda, .buddy, .cedric, .dave, .grace, .john, .matt, .meagan, .pat, .richard, .travis"
          ).hide();
          break;
        case "Amanda":
          $(
            ".adam, .all, .buddy, .cedric, .dave, .grace, .john, .matt, .meagan, .pat, .richard, .travis"
          ).hide();
          break;
        case "Buddy":
          $(
            ".adam, .amanda, .all, .cedric, .dave, .grace, .john, .matt, .meagan, .pat, .richard, .travis"
          ).hide();
          break;
        case "Cedric":
          $(
            ".adam, .amanda, .buddy, .all, .dave, .grace, .john, .matt, .meagan, .pat, .richard, .travis"
          ).hide();
          break;
        case "Dave":
          $(
            ".adam, .amanda, .buddy, .cedric, .all, .grace, .john, .matt, .meagan, .pat, .richard, .travis"
          ).hide();
          break;
        case "Grace":
          $(
            ".adam, .amanda, .buddy, .cedric, .dave, .all, .john, .matt, .meagan, .pat, .richard, .travis"
          ).hide();
          break;
        case "John":
          $(
            ".adam, .amanda, .buddy, .cedric, .dave, .grace, .all, .matt, .meagan, .pat, .richard, .travis"
          ).hide();
          break;
        case "Matt":
          $(
            ".adam, .amanda, .buddy, .cedric, .dave, .grace, .john, .all, .meagan, .pat, .richard, .travis"
          ).hide();
          break;
        case "Meagan":
          $(
            ".adam, .amanda, .buddy, .cedric, .dave, .grace, .john, .matt, .all, .pat, .richard, .travis"
          ).hide();
          break;
        case "Pat":
          $(
            ".adam, .amanda, .buddy, .cedric, .dave, .grace, .john, .matt, .meagan, .all, .richard, .travis"
          ).hide();
          break;
        case "Richard":
          $(
            ".adam, .amanda, .buddy, .cedric, .dave, .grace, .john, .matt, .meagan, .pat, .all, .travis"
          ).hide();
          break;
        case "Travis":
          $(
            ".adam, .amanda, .buddy, .cedric, .dave, .grace, .john, .matt, .meagan, .pat, .richard, .all"
          ).hide();
          break;
      }
    });
  ...
  $(".addBaseline").on("click", function(e) {
    ...
    $.ajax({
      ...
      $(`.editBaseline[name=${name}]`).attr({
        id: response.id,
        "data-comm-name": commodity,
      });
      $(`.deleteBaseline[name=${name}]`).attr({
        id: response.id,
        "data-comm-name": commodity,
      });
      $(this).hide(),
  ...
  $(".addPredicted").on("click", function(e) {
    ...
    $.ajax({
      ...
      $(`.editPredicted[name=${name}]`).attr({
        id: response.id,
        "data-comm-name": commodity,
      });
      $(`.deletePredicted[name=${name}]`).attr({
        id: response.id,
        "data-comm-name": commodity,
      });
      $(this).hide(),
  ...
  $(".addMv").on("click", function(e) {
    ...
    $.ajax({
      ...
      $(`.editMv[name=${name}]`).attr({
        id: response.id,
        "data-comm-name": commodity,
      });
      $(`.deleteMv[name=${name}]`).attr({
        id: response.id,
        "data-comm-name": commodity,
      });
      $(this).hide(),
  ...
  $("#formData").on("click", "button.updateCost", function() {
    ...
    if (!data.cost) {
      errors.push({
        text: "Please enter a value for costs (if value is unknown, enter 0)",
      });
    }
    if (!data.hours) {
      errors.push({
        text: "Please enter a value for hours (if value is unknown, enter 0)",
      });
    }
  ...
  $("#formData").on("click", "button.updateFunding", function() {
    ...
    if (!data.implementation) {
      errors.push({
        text:
          "Please enter a value for implementation (if value is unknown, enter 0)",
      });
    }

    if (!data.annual) {
      errors.push({
        text: "Please enter a value for annual (if value is unknown, enter 0)",
      });
    }
  ...
  $("#formData").on("click", "button.updateMisc", function() {
    ...
    if (!data.misc_savings) {
      errors.push({
        text:
          "Please enter a value for Misc Savings (if value is unknown, enter 0)",
      });
    }
  ...
  $table.on("click", "button.addRow", function(e) {
    ...
    if (!data.imp_or_ann) {
      errors.push({
        text: "Please select an option for implementation or annual",
      });
    }
    if (!data.category) {
    ...
    if (!data.cost) {
      errors.push({
        text: "Please enter a value for cost (if value is unknown, enter 0)",
      });
    }
    if (!data.hours) {
      errors.push({
        text: "Please enter a value for hours (if value is unkown, enter 0)",
      });
    }
