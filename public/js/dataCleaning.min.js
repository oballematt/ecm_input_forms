let analysisIndex,meterData=[],reviewedModels=[],replaceData=[];const d=new Date;let updateModelStart,updateModelEnd;$(document).ready(()=>{const a=$(".datepicker");a.datepicker({changeYear:!0,dateFormat:"yy-mm-dd"}),$(".load").on("click",function(){$("#overlay").fadeIn()});const b=new Date(d.getFullYear(),d.getMonth()+1,1);d===b&&$.ajax({url:"/deleteModels",type:"DELETE"});const c=()=>{$.when($.ajax({url:"/getAlarm",type:"POST",data:{startTimestamp:new Date(d.getFullYear(),d.getMonth()-1,1).toISOString().slice(0,10),endTimestamp:new Date(d.getFullYear(),d.getMonth(),0).toISOString().slice(0,10),steward:$("#filter-steward").val()}}),$.ajax({url:"/reviewedModels",type:"GET"})).then((a,b)=>{if("Request failed with status code 504"===a[0]||"Request failed with status code 500"===a[0])c();else{console.log(b),$(".ring").hide(),$(".apply").html("Apply");const c=a[0].body.meter,d=a[0].body.flag_count,e=a[0].body.building_abbreviation,f=a[0].body.building_number,g=a[0].body.commodity_tag;c.map((a,b)=>{$(".meterList").append(`
            <tr>
