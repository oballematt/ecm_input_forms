...
$(document).ready(() => {
    ...
    const modelApi = function () {
        ...
        $.ajax({
            ...
            $('#tableData').show()
            lowValueArray.push(obj.model.data.predicted_value_lower_bound)
            let lowValueSliceArray = lowValueArray[0].slice(365)
            xTemp.push(obj.model.data.average_dry_bulb_temperature)
            let xTempSlice = xTemp[0].slice(365)
            xTempSlice.forEach((key, i) => result[key] = lowValueSliceArray[i])
            var lowArray = Object.keys(result).map(function (key) {
                return [Number(key), result[key]];
            })
            lowArray.sort(function (a, b) {
                return a[0] - b[0]
            })
                ...
                let data = [
                    {
                        ...
                        'Replacement': replacementValue[365],
                        'Reason': replacementReason[365],
                        'Notes': replacementNotes[365],
                    },
                    {
                        ...
                        'Replacement': replacementValue[366],
                        'Reason': replacementReason[366],
                        'Notes': replacementNotes[366],
                    },
                    {
